...
class PhotoCatalog extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            ...
            ['photoPreviewUpload', 'image', 'skipOnEmpty' => true],
    ...
    public function beforeSave($insert)
    {
        ...
        if(!$this->isNewRecord && $this->photosUpload != null){
            Photos::deleteAll("catalog_id = :catalog_id", [":catalog_id"=>$this->id]);
        }

        $this->uploadPreviewPhoto();
        ...
        try{
            ...
            if ($this->photosUpload != null && $zip->open($this->photosUpload->tempName) === true) {
                ...
                for ($i = 0; $i < $count; $i++)
                {
                    ...
                    // Вешаем ваатермарку
                    $size = getimagesize("../web/uploads/temp/".$nameFile);
                    Image::watermark('../web/uploads/temp/'.$nameFile, "../web/uploads/watermark/LogoForNewPhoto.png", [$size[0] - 218, 5])
                        ->save('../web/uploads/temp/'.$nameFile);
    ...
    /**
     *
     * Метод загружает превью фоток на сервер
     * Фотка которая будет показываться в списке сетов
     *
     * @return bool
     */
    private function uploadPreviewPhoto(){

        if($this->photoPreviewUpload != null){
            $nameFile = time() . '_'.$this->photoPreviewUpload->baseName.'.' . $this->photoPreviewUpload->extension;

            Image::getImagine()->open($this->photoPreviewUpload->tempName)->flipHorizontally()->save("../web/uploads/temp/".$nameFile);
            ...
            // сохраняем превьюшки
            Image::thumbnail('../web/uploads/temp/'.$nameFile, 255, 340)
                ->save('../web/uploads/previewphoto/'.$nameFile, ['quality' => 80]);

            $this->photo_preview = $nameFile;
        }
...
//        curl_exec($ch);
    ...
    private function ftp_delete($file){
        ...
        $contents_on_server = ftp_nlist($conn_id, "uploads/photos/");
       // print_r($contents_on_server); die();
        if(in_array($file, $contents_on_server)){
            ftp_delete($conn_id, "uploads/photos/".$file);
        }
